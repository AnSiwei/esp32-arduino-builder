...
# Fixed URL of the prebuilt Arduino framework package (mirrors platform.json).
# The CI replaces the file behind this URL on every successful build; freshness
# is enforced by Espressif32Platform._ensure_framework_fresh() which compares a
# remote fingerprint asset with the locally cached one.
FRAMEWORK_PKG_NAME = "framework-arduinoespressif32"
FRAMEWORK_FINGERPRINT_URL = (
    "https://github.com/AnSiwei/esp32-arduino-builder"
    "/releases/download/framework-latest/framework-fingerprint.json"
)
FINGERPRINT_CACHE_FILE = "framework-fingerprint.remote.json"
...
class Espressif32Platform(PlatformBase):
    ...
    def get_toolchain_bin_dir(self, tool_name: str) -> Optional[str]:
        """
        Return the actual directory containing toolchain executables (bin dir).

        The xtensa toolchain installed via ESP-IDF idf_tools has a nested layout
        ``<pkg>/xtensa-esp-elf/bin/`` (see tools.json ``export_paths``), while the
        riscv toolchain from the PlatformIO registry is flat ``<pkg>/bin``.
        PlatformIO core only adds ``<pkg>/bin`` (falling back to ``<pkg>``) to
        $PATH, so the nested Xtensa binaries would not be found. This helper
        resolves both layouts.

        Args:
            tool_name: Package name, e.g. "toolchain-xtensa-esp-elf".

        Returns:
            Path to the bin directory or None if the package is not installed.
        """
        pkg_dir = self.get_package_dir(tool_name)
        if not pkg_dir:
            return None

        pkg_path = Path(pkg_dir)
        # Flat layout: <pkg>/bin
        flat_bin = pkg_path / "bin"
        if flat_bin.is_dir():
            return str(flat_bin)

        # Nested layout: <pkg>/<arch>/bin (e.g. xtensa-esp-elf/bin)
    ...
    def _ensure_framework_fresh(self) -> None:
        """Detect a new CI build behind the fixed framework package URL.

        Compares the remote framework-fingerprint.json with the locally cached
        one. On a change: clear the PlatformIO download-cache entry for the
        framework URL and uninstall the installed package so the next package
        resolution downloads the fresh archive.
        """
        fw_spec = self.packages.get(FRAMEWORK_PKG_NAME, {}).get("version")
        if not fw_spec or not fw_spec.startswith("http"):
            logger.debug("Framework package is not URL-based; skip fingerprint check")
            return
        ...
        try:
            ...
            req = urllib.request.Request(
                FRAMEWORK_FINGERPRINT_URL, headers={"Accept": "application/json"}
            )
            with urllib.request.urlopen(req, timeout=30) as resp:
                remote_fp = json.load(resp)
        except (OSError, ValueError) as e:
            logger.debug("Remote framework fingerprint unavailable: %s", e)
            return

        remote_fp_value = (
            remote_fp.get("sha256") if isinstance(remote_fp, dict) else None
        )
        if not remote_fp_value:
            logger.debug("Remote framework fingerprint has no sha256; skip")
            return

        cache_path = Path(self.packages_dir) / FINGERPRINT_CACHE_FILE
        local_fp_value = None
        if cache_path.is_file():
            try:
                local_fp_value = json.loads(
                    cache_path.read_text(encoding="utf-8")
                ).get("sha256")
            except (OSError, ValueError):
                local_fp_value = None

        if local_fp_value == remote_fp_value:
            logger.debug("Framework fingerprint unchanged (%s)", remote_fp_value)
            return

        # Fingerprint changed: clear the PM download cache (keyed by URL hash)
        # and uninstall the installed framework package.
        try:
            dl_dir = Path(pm.get_download_dir())
            request_hash = hashlib.new("sha1")
            request_hash.update(fw_spec.encode("utf-8"))
            cached = dl_dir / request_hash.hexdigest()
            if cached.is_file():
                cached.unlink()
                logger.info("Removed stale framework download cache: %s", cached)
        except Exception as e:  # pylint: disable=broad-except
            logger.debug("Could not clear framework download cache: %s", e)

        installed_dir = self.get_package_dir(FRAMEWORK_PKG_NAME)
        if installed_dir:
            logger.info(
                "Framework build changed (fingerprint %s -> %s); reinstalling",
                local_fp_value,
                remote_fp_value,
            )
            try:
                pm.uninstall(fw_spec)
            except Exception as e:  # pylint: disable=broad-except
                logger.warning(
                    "Could not uninstall stale framework package: %s", e
                )
                safe_remove_directory(Path(installed_dir))

        try:
            cache_path.parent.mkdir(parents=True, exist_ok=True)
            cache_path.write_text(
                json.dumps(remote_fp, ensure_ascii=False, indent=2) + "\n",
                encoding="utf-8",
            )
        except OSError as e:
            logger.debug("Could not persist framework fingerprint: %s", e)
    ...
    # 只保留这三家厂商的板子（其余板子文件保留在仓库，但不出现在最终产物中）
    ALLOWED_VENDORS = ("Espressif", "M5Stack", "Seeed Studio")

    def get_boards(self, id_=None):
        """Get board configuration with dynamic options, filtered by allowed vendors."""
        result = super().get_boards(id_)
        if not result:
            return result
        if id_:
            # 指定板子 ID 时也校验厂商，非三家厂商的板子视为不存在
            vendor = result.manifest.get("vendor", "")
            ...
            for key, value in result.items():
                vendor = value.manifest.get("vendor", "")
                if vendor in self.ALLOWED_VENDORS:
                    filtered[key] = self._add_dynamic_options(value)
            return filtered
