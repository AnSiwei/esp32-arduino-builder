...
# Ensure the toolchain bin directory is on $PATH.
# PlatformIO core only prepends "<pkg>/bin" (or "<pkg>" when bin is missing),
# but the ESP-IDF installed Xtensa toolchain has a nested layout
# "<pkg>/xtensa-esp-elf/bin". Without this the compiler is not found.
_toolchain_pkg = (
    "toolchain-xtensa-esp-elf"
    if is_xtensa
    else "toolchain-riscv32-esp"
)
_toolchain_bin = platform.get_toolchain_bin_dir(_toolchain_pkg)
if _toolchain_bin and os.path.isdir(_toolchain_bin):
