...
# Actual bin directory. The ESP-IDF installed Xtensa toolchain is nested
# ("<pkg>/xtensa-esp-elf/bin"), while the RISC-V one is flat ("<pkg>/bin").
# All later code assumes "<bin>/<tool>" layout, so resolve the real bin dir.
TOOLCHAIN_BIN_DIR = platform.get_toolchain_bin_dir(
    "toolchain-xtensa-esp-elf"
    if mcu in ("esp32", "esp32s2", "esp32s3")
    else "toolchain-riscv32-esp"
)
...
if not TOOLCHAIN_BIN_DIR or not os.path.isdir(TOOLCHAIN_BIN_DIR):
    sys.stderr.write(f"Error: Missing toolchain bin directory '{TOOLCHAIN_BIN_DIR}'\n")
    env.Exit(1)
...
def generate_project_ld_script(sdk_config, ignore_targets=None):
    ...
    args = {
        ...
        "objdump": str(Path(TOOLCHAIN_BIN_DIR) / env.subst("$CC").replace("-gcc", "-objdump")),
    }
    ...
    if framework_version_list[0] >= 6:
        ...
        return env.Command(
            ...
            env.VerboseAction(
                " ".join([
                    f'"{CMAKE_DIR}"',
                    f'-DCC="{fs.to_unix_path(str(Path(TOOLCHAIN_BIN_DIR) / "$CC"))}"',
        ...
        return env.Command(
            ...
            env.VerboseAction(
                " ".join([
                    f'"{CMAKE_DIR}"',
                    f'-DCC="{str(Path(TOOLCHAIN_BIN_DIR) / "$CC")}"',
                    "-DSOURCE=$SOURCE",
                    "-DTARGET=$TARGET",
