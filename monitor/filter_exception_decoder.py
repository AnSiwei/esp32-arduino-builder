        ...
        try:
            ...
            for tool_name in tool_names:
                ...
                for pkg_name in pkg_name_candidates:
                    ...
                    if pkg and pkg.path:
                        # Xtensa toolchain from idf_tools is nested
                        # (<pkg>/xtensa-esp-elf/bin), RISC-V is flat (<pkg>/bin)
                        for bin_dir in (str(Path(pkg.path) / "bin"), str(Path(pkg.path))):
                            tool_bin = str(Path(bin_dir) / tool_name)
                            if IS_WINDOWS:
                                tool_bin += ".exe"
                            if os.path.isfile(tool_bin):
                                return tool_bin
                        for sub in Path(pkg.path).iterdir():
                            if sub.is_dir():
                                nested_bin = sub / "bin"
                                tool_bin = str(nested_bin / tool_name)
                                if IS_WINDOWS:
                                    tool_bin += ".exe"
                                if os.path.isfile(tool_bin):
                                    return tool_bin
            ...
            for tool_name in tool_names:
                ...
                for pkg_name in pkg_name_candidates:
                    ...
                    if os.path.isdir(pkg_dir):
                        ...
                        for candidate_dir in (bin_dir,):
                            candidate = os.path.join(candidate_dir, tool_name)
                            if IS_WINDOWS:
                                candidate += ".exe"
                            if os.path.isfile(candidate):
                                return candidate
                        # Nested: <pkg>/<arch>/bin (Xtensa from idf_tools)
                        for sub in Path(pkg_dir).iterdir():
                            if sub.is_dir():
                                nested_bin = sub / "bin"
                                candidate = str(nested_bin / tool_name)
                                if IS_WINDOWS:
                                    candidate += ".exe"
                                if os.path.isfile(candidate):
                                    return candidate
